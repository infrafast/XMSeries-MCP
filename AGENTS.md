...
- **Runtime mixer changes**: The server starts from environment variables, then `osc_configure_mixer` can change host, port, protocol, or channel/bus/FX/DCA counts. Omitted fields keep their current values; count-only changes do not reconnect.
