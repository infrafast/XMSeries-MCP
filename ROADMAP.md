...
Status: **deterministic parser V1 CLOSED on 2026-09-21. The unified native parser, generic named-list inference, multi-destination composition and explicit family qualifiers are merged on `main`; Node 20.20/22 CI and live Pi mixer acceptance are green. Remaining OR4/XDG work below is runtime/cross-repository acceptance, not parser-architecture work.**
...
- [~] source -> destination bus routes: Local parsing supports channel, FX return and aux return sources to a bus for deterministic reads plus absolute, explicit-relative, qualitative, mute/unmute, ramp and delayed writes, reusing the corresponding cloud/OSC primitives and adaptive `osc_adjust_level` behavior. Route reads are classified `effect=read`; route mute remains protocol-aware through the existing OSC helpers. Live route acceptance remains pending;
...
- [~] fades/ramps and delayed actions through the existing MCP-side automation engine: absolute/explicit-relative ramps, fades, delayed level changes, single-target delayed mute/unmute, source-to-bus ramps and delayed source-to-bus mute/unmute are implemented. Delayed mute delegates to the same protocol-aware mute adapters after the AutomationEngine delay, including channel/FX/aux route-mute helpers, so existing XR/X32 unsupported-operation guards remain authoritative. The qualitative-ramp parity slice also previews the same adaptive `osc_adjust_level` target for `monte/baisse [un peu/beaucoup] progressivement` before starting the ramp. Live Pi validation confirms the physical progressive/delayed actions; a 21 September OSCXR status run exposed stale immediate readback after delayed one-shot level writes, so delayed verification now retries bounded readback propagation before declaring failure. Final live recheck plus newer delayed-mute/qualitative/source-to-bus acceptance remain pending;
...
- [x] **PARSER V1 CLOSED:** every elementary Local command that can be represented as lexical slots + semantic constraints enters the same Node/TypeScript matcher. The retired gateway regex front-end and parser-only fallbacks are removed. Sequence/anaphora composition and recognized-speaker expansion remain separate by design, while every elementary clause still re-enters the same parser. Resolver identity, capability checks, ambiguity/fuzzy-write safety, stale-target validation, XR/X32 guards, OSC paths and automation execution remain specialized domain layers.
- [x] generic explicit family qualifiers: target prefixes such as `bus`, `channel/voie/canal/tranche`, `fx/effect`, `aux`, `dca` and `matrix/matrice` are resolver constraints, not label text. Structured ownership phrases such as `channel guitare de anto` keep ownership matching; contradictory qualifiers fail closed; real labels beginning with a family word such as `Matrix Vox` are preserved by trying the complete label first.
- [x] conservative phonetic target identity fallback: after exact/contains/structured resolution, target names may match by a deterministic French-oriented phonetic signature (for example `Anto` / `ento` / `en taux`, `Mika` / `Mica` / `Micka`). This stage is target-only, family-scoped, exact on the phonetic signature and safe only when unique; multiple phonetic candidates fail closed and edit-distance fuzzy matches remain non-authoritative for writes.
- [x] generic named-list / multi-destination composition: unqualified `A et B` target lists and `source → A et B` destination lists resolve name-by-name before family/capability validation. The same wrapper covers send read, absolute/relative/qualitative writes, mute, delay and ramp instead of command-specific grammar.
- [x] parser extension contract frozen: future mixer capabilities must extend `NativeMixerIntent` / the existing lexical-slot matcher and converge with the typed MCP surface on the same resolver/business/OSC/automation primitives. `AGENTS.md` makes Cloud/Local symmetry, regression corpus/recipe updates, protocol-safety checks and documentation updates mandatory. New whole-utterance parser paths in the gateway or LiveStageAssistant are architecture regressions unless explicitly documented as migrations.
...
- [~] include STT-like case/punctuation variants without introducing unconstrained fuzzy NLP: canonical French cases include safe `montre`→`monte` correction, `plus fort` / `moins fort`, cloud-documented mute aliases (`éteins`, `rallume`, `active`, `ouvre`), Main aliases (`front`, `principal`) and `son` as a level noun, with explicit display/read forms kept fail-closed; broader variants remain incremental;
...
- [~] maintain an isolated STT replay scorer for cross-repo engine benchmarks: `scripts/stt_resolver_benchmark.mjs` consumes transcription JSON only, imports the current production parser/resolver, compares the real XR16 registry plus a 60-name stress registry, records phonetic recoveries and `wrong_accepted`, and performs no OSC/MCP/mixer I/O. The 25 September regenerated 48-WAV corpus exposed three concrete Local issues: bounded `mais` -> `mets` action leakage into target queries, `de mute, baisse, Mike` collapsing unsafely to exact bus `Mike`, and exact STT spellings that can hide a phonetic collision in a larger registry. Production Local parsing/resolution now repairs the bounded action forms, treats `mic`/spoken `baisse Mike` as phonetic variants of `basse-mike`, and enables an exact-vs-phonetic collision guard only on the Local gateway while preserving normal typed MCP exact-match behavior. Replay of the same saved STT JSON is pending to quantify the post-fix gain; no WAV re-recording is required.
