...
- [x] native constraint-driven parser cutover: ordinary Local level/routing/ramp/delay/mute language now uses the Node/TypeScript lexical-slot extractor plus semantic constraint matcher with no generic whole-utterance regex fallback. Existing resolver, ambiguity/fuzzy-write safety, planner, OSC adapters and automation engine remain authoritative; only specialized metadata/state, physical-AUX, grouped/bulk, automation-control and sequence grammars remain outside the matcher. Node 20.20/22 gateway, corpus and functional-recipe regression CI validates the cutover; live Pi acceptance remains tracked separately in XDG4.
