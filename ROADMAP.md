...
- [x] final native parser consolidation: every elementary Local command that can be represented as lexical slots + semantic constraints now enters the same Node/TypeScript matcher, including mixer/automation status, property reads, explicit FX-engine state, grouped/all/all-except selectors, bulk sends, physical-AUX destinations, automation cancellation, ordinary level/routing/mute and timed commands. The retired gateway regex front-end and its parser-only helpers are removed. Only temporal sequence composition/explicit anaphora and recognized-speaker expansion remain outside the elementary matcher by design; resolver identity, capability checks, ambiguity/fuzzy-write safety, stale-target validation, XR/X32 guards, OSC paths and automation execution remain specialized domain layers. Typed MCP parity now also exposes the FX-return fader and reuses the existing effect on/off primitive. CI plus final live Pi acceptance are tracked before closing XDG4.
- [x] generic explicit family qualifiers: target prefixes such as `bus`, `channel/voie/canal/tranche`, `fx/effect`, `aux`, `dca` and `matrix/matrice` are resolved as family constraints across standalone targets and route endpoints. The qualifier is removed before name resolution, so structured names such as `channel guitare de anto` still use ownership matching; contradictory qualifiers fail closed.
- [x] named-list inference hardening after live Pi acceptance: unqualified `A et B` target lists and `source → A et B` destination lists are resolved name-by-name before family/capability validation. The composition wraps every send intent (read, absolute/relative/qualitative write, mute, delay and ramp) rather than adding command-specific grammar. Exact labels containing a conjunction are preserved, mixed-family target mute lists use the generic target mute primitive, and unsupported send destination families fail closed after successful name resolution.
