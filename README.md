...
Resolver order is **exact -> contains -> structured ownership -> phonetic -> fuzzy**. The phonetic stage exists specifically for STT spelling drift where the spoken identity is preserved but the transcript spelling changes, for example `Anto` -> `ento` / `en taux`, `Mika` -> `Mica` / `Micka`, `Mike` -> `mic`, `baisse Mike` -> `basse-mike`, or `guitare de ento` -> `guitar-anto`. It compares deterministic phonetic signatures of the already-extracted target name; it never rewrites signs, numeric values or units.

A phonetic match is authorized only when it is unique inside the resolver's current family scope. Multiple phonetic matches fail closed and require clarification, exactly like other ambiguous identity cases. Ordinary edit-distance `fuzzy` matches remain advisory only and cannot authorize a write. The Local deterministic/STT gateway adds one stricter safety rule: if the transcript is an exact label but another candidate in the same active family scope has the same phonetic signature, Local resolution treats the identity as ambiguous instead of letting the exact spelling win. This protects cases such as spoken `guitar Laurent` when both `guitar-laurent` and phonetic peer `guitar-loran` exist. The normal typed MCP resolver keeps its historical exact-match priority. In a phrase such as `monte la guitare de Anto sur Laurent`, resolve the complete ownership phrase in the `channel` family and resolve `Laurent` separately in the `bus` family.

Local STT canonicalization also contains bounded repairs for observed transcription artifacts: leading `mais` is interpreted as imperative `mets` only when the same utterance already contains an explicit absolute dB/% value, and prefix `de mute` is normalized as `démute`. Mute/unmute extraction happens before directional level verbs so an STT target such as `baisse Mike` remains a target phrase instead of collapsing to the unrelated exact bus `Mike`.
