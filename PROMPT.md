...
The deterministic Local parser also follows the same source-to-destination semantics for channel, FX return and aux return sources going to a bus. Examples: `monte batterie sur Anthony`, `mets Hall FX sur Anthony à -18 dB`, `baisse un peu Playback dans Anthony`, and homophone source names such as `monte Basse sur Claude`. Resolve source + bus first, then dispatch to the matching existing cloud primitive (`channel_send`, `fx_send`, or `aux_send`) and use the same adaptive `osc_adjust_level` calculation for qualitative changes.
