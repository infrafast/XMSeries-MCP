...
The deterministic Local parser also follows the same source-to-destination semantics for channel, FX return and aux return sources going to a bus. Examples: `niveau de batterie sur Anthony`, `monte batterie sur Anthony`, `mets Hall FX sur Anthony à -18 dB`, `baisse un peu Playback dans Anthony`, and homophone source names such as `monte Basse sur Claude`. Resolve source + bus first. Read forms such as `niveau de <source> sur <bus>` must remain `effect=read` and use the matching existing get-send primitive; writes dispatch to the matching existing cloud primitive (`channel_send`, `fx_send`, or `aux_send`) and use the same adaptive `osc_adjust_level` calculation for qualitative changes. `mute/unmute <source> sur|dans <bus>` must use the dedicated route-mute primitive for that source family and must never broaden into whole-source mute. Delayed route mute/unmute such as `mute Batterie sur Anthony dans 5 secondes` or `dans 5 secondes, réactive Playback dans Anthony` must schedule that same route-mute primitive through the MCP-side AutomationEngine; do not convert it into a source mute or a level automation.

The deterministic Local parser follows the same domain semantics when the host supplies generic speaker context. Canonical first-person phrases include `monte mon retour de 3 dB`, `mets mon micro à -12 dB`, and `mets batterie dans mon retour à -20 dB`. Resolve monitor destination explicitly from `XMS_SPEAKER_MAP`: mapped `bus` -> that bus; explicitly mapped speaker without `bus` -> Main LR/façade; speaker absent from the map -> unresolved/clarify. For a Main destination, a source-level phrase such as `batterie dans mon retour` uses the source Main LR fader path, not a synthetic bus. Keep mute semantics conservative: `mute mon retour` may mute Main LR, but `mute batterie dans mon retour` must clarify/fail closed because a dedicated source→Main send mute is not exposed. Unknown speakers or missing input-channel mappings must clarify rather than guess.
...
Natural direction aliases `plus fort` / `moins fort` map to the same adaptive qualitative up/down semantics, including named targets and channel-to-bus routes.
